<?php
	...
	if ($method == "page") {
		...
		echo $view->parse($_DOCUMENT_ROOT . $_PACKAGE . 
				(isset($_GET["friendly"]) ? "/html/@_PAGE.html" : "/html/telas.html"), $response);
		...
		if (!getActiveSession($_ROOT . $_MODULE)) 
			...
			if ($method == "stateCreate") {
				if (!getPermission($_ROOT . $_MODULE, $daoFactory, $screen, $method))
					...
					$response["menus"] = $daoFactory->getMenusDao()->read("", "menus.id ASC", false);
					$daoFactory->close();
			 ...
			 */
				...
				if (!getPermission($_ROOT . $_MODULE, $daoFactory, $screen, $method))
					...
					$response["telas"][0]["telas.menus"] = $daoFactory->getMenusDao()->read("", "menus.id ASC", false);
